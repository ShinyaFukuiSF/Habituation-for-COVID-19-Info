...
### 1. 1st <--- start with this (read and create data)(R version is to be 4.1 and later)
### 2. 2nd_1,2,3  <-- W*X_2020, W*X_2021, and W*X_2022
###                    (create a cross term of spatial weight matrix and explanatory variable, W*X)
###                    Weight matricies are calculated in advance.        
### 3. 3rd <-- before estimation (combining and organising data)
### 4. 4th <-- estimation using phtt (change R version to 4.0)  (estimation)
###########################################################################################################
...
vaccine <- vaccine %>% dplyr::group_by(pref_no, date) %>% 
  mutate(count_sum_1 = sum(count_1), count_sum_2 = sum(count_2), count_sum_3 = sum(count_3)) 
...
vaccine <- vaccine %>% dplyr::mutate_at(vars(-group_cols()), ~replace(., is.na(.), 0))
...
vaccine <- vaccine %>% tidyr::complete(date = seq(as.Date("2020-02-14"), as.Date("2021-04-11"), by="day"))  ### before 2021-04-11, there are no dose, so fill in with 0s
...
jinko <- read.csv("jinko(population).csv") 
...
vaccine <- vaccine %>% dplyr::group_by(pref_no) %>% 
  mutate(vaccine_rate_1_diff = (vaccine_rate_1 - dplyr::lag(vaccine_rate_1, n=1))) %>% 
  mutate(vaccine_rate_2_diff = (vaccine_rate_2 - dplyr::lag(vaccine_rate_2, n=1))) %>% 
  mutate(vaccine_rate_3_diff = (vaccine_rate_3 - dplyr::lag(vaccine_rate_3, n=1))) %>% 
  rowwise
vaccine <- vaccine %>% dplyr::group_by(pref_no) %>% 
  ...
  rowwise 
vaccine <- vaccine %>% dplyr::group_by(pref_no) %>% 
  mutate(vaccine_rate_1_diff7_diff = (vaccine_rate_1_diff7 - dplyr::lag(vaccine_rate_1_diff7, n=1))) %>% 
  mutate(vaccine_rate_2_diff7_diff = (vaccine_rate_2_diff7 - dplyr::lag(vaccine_rate_2_diff7, n=1))) %>% 
  mutate(vaccine_rate_3_diff7_diff = (vaccine_rate_3_diff7 - dplyr::lag(vaccine_rate_3_diff7, n=1))) %>% 
  rowwise 
vaccine <- vaccine %>% dplyr::group_by(pref_no) %>% 
  ...
  rowwise 
vaccine <- vaccine %>% dplyr::group_by(pref_no) %>% 
  mutate(ihs_vaccine_1_diff7 = (ihs_vaccine_1 - dplyr::lag(ihs_vaccine_1, n=7))) %>% 
  mutate(ihs_vaccine_2_diff7 = (ihs_vaccine_2 - dplyr::lag(ihs_vaccine_2, n=7))) %>% 
  mutate(ihs_vaccine_3_diff7 = (ihs_vaccine_3 - dplyr::lag(ihs_vaccine_3, n=7))) %>% 
  rowwise 
vaccine <- vaccine %>% dplyr::group_by(pref_no) %>% 
  mutate(ihs_vaccine_1_diff = (ihs_vaccine_1 - dplyr::lag(ihs_vaccine_1, n=1))) %>% 
  mutate(ihs_vaccine_2_diff = (ihs_vaccine_2 - dplyr::lag(ihs_vaccine_2, n=1))) %>% 
  mutate(ihs_vaccine_3_diff = (ihs_vaccine_3 - dplyr::lag(ihs_vaccine_3, n=1))) %>% 
  rowwise 
vaccine <- vaccine %>% dplyr::group_by(pref_no) %>% 
  ...
  rowwise 
vaccine <- vaccine %>% dplyr::group_by(pref_no) %>% 
  mutate(ihs_count_cumsum_1_diff7 = (ihs_count_cumsum_1 - dplyr::lag(ihs_count_cumsum_1, n=7))) %>% 
  mutate(ihs_count_cumsum_2_diff7 = (ihs_count_cumsum_2 - dplyr::lag(ihs_count_cumsum_2, n=7))) %>% 
  mutate(ihs_count_cumsum_3_diff7 = (ihs_count_cumsum_3 - dplyr::lag(ihs_count_cumsum_3, n=7))) %>% 
  rowwise 
vaccine <- vaccine %>% dplyr::group_by(pref_no) %>% 
  mutate(ihs_count_cumsum_1_diff = (ihs_count_cumsum_1 - dplyr::lag(ihs_count_cumsum_1, n=1))) %>% 
  mutate(ihs_count_cumsum_2_diff = (ihs_count_cumsum_2 - dplyr::lag(ihs_count_cumsum_2, n=1))) %>% 
  mutate(ihs_count_cumsum_3_diff = (ihs_count_cumsum_3 - dplyr::lag(ihs_count_cumsum_3, n=1))) %>% 
  rowwise 
vaccine <- vaccine %>% dplyr::group_by(pref_no) %>% 
  mutate(count_cumsum_1_diff7  = (count_cumsum_1 - dplyr::lag(count_cumsum_1, n=7))) %>% 
  mutate(count_cumsum_2_diff7  = (count_cumsum_2 - dplyr::lag(count_cumsum_2, n=7))) %>%   
  mutate(count_cumsum_3_diff7  = (count_cumsum_3 - dplyr::lag(count_cumsum_3, n=7))) %>% 
  rowwise 
vaccine <- vaccine %>% dplyr::group_by(pref_no) %>% 
  mutate(count_cumsum_1_diff  = (count_cumsum_1 - dplyr::lag(count_cumsum_1, n=1))) %>% 
  mutate(count_cumsum_2_diff  = (count_cumsum_2 - dplyr::lag(count_cumsum_2, n=1))) %>%   
  mutate(count_cumsum_3_diff  = (count_cumsum_3 - dplyr::lag(count_cumsum_3, n=1))) %>% 
  rowwise 
...
###############
## precipitation
precipitation <- read.csv("precipitation_renamed.csv")
precipitation <- precipitation %>% pivot_longer(cols= c(-date), names_to = "pref", values_to = "precipitation")
...
## temperature
temperature <- read.csv("temperature_renamed.csv")
temperature <- temperature %>% pivot_longer(cols= c(-date), names_to = "pref", values_to = "temperature")
...
##########################
vaccine <- vaccine %>%
  dplyr::select(pref_no, date, count_cumsum_1, count_cumsum_2, count_cumsum_3,
                ihs_count_cumsum_1, ihs_count_cumsum_2, ihs_count_cumsum_3,
                ihs_count_cumsum_1_diff7, ihs_count_cumsum_2_diff7, ihs_count_cumsum_3_diff7,
                ihs_count_cumsum_1_diff, ihs_count_cumsum_2_diff, ihs_count_cumsum_3_diff,
                ihs_vaccine_1_diff, ihs_vaccine_2_diff, ihs_vaccine_3_diff,
                count_cumsum_1_diff7, count_cumsum_2_diff7, count_cumsum_3_diff7,
                count_cumsum_1_diff, count_cumsum_2_diff, count_cumsum_3_diff,
                vaccine_rate_1, vaccine_rate_2, vaccine_rate_3, 
                vaccine_rate_1_diff, vaccine_rate_2_diff, vaccine_rate_3_diff,  
                vaccine_rate_1_diff7, vaccine_rate_2_diff7, vaccine_rate_3_diff7,
                vaccine_rate_1_diff7_diff, vaccine_rate_2_diff7_diff, vaccine_rate_3_diff7_diff,
                ihs_vaccine_1, ihs_vaccine_2, ihs_vaccine_3,
                ihs_vaccine_1_diff7, ihs_vaccine_2_diff7, ihs_vaccine_3_diff7) %>% 
  filter(date >= as.Date("2020-02-21") & date <= as.Date("2022-6-20")) %>% arrange(as.Date(date)) %>% 
  rename (date_vac = date)


temperature <- temperature %>%
  dplyr::select(pref_no, date, ihs_temperature, ihs_temperature_diff) %>% 
  filter(date >= as.Date("2020-02-21") & date <= as.Date("2022-6-20")) %>% arrange(as.Date(date)) %>% 
...
#####################
## pop_density
pop_dens <- read.csv("pop_dens_renamed.csv") 
...
## over65
over_65 <- read.csv("over65_renamed.csv") 
